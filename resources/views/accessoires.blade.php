    ...
    <div class="w-full md:w-4/5 bg-[#F1F4F3] mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            @foreach ($fietsen as $fiets)
                <div class="bg-[#cdcdcd] flex flex-col items-center p-3 product-item cursor-pointer" data-name="{{ $fiets->Naam }}" data-price="{{ $fiets->Prijs }}" data-image="{{ Vite::asset($fiets->AfbeeldingPad) }}" data-description="{{ $fiets->Beschrijving }}" data-rijhoogte="{{ $fiets->RijHoogte }}">
                    <img src="{{ Vite::asset($fiets->AfbeeldingPad) }}" alt="{{ $fiets->Naam }}" class="w-full h-64 object-contain">
                    <div class="w-full flex flex-col">
